...
void max_jit_tml_fluid2_assist(t_max_jit_tml_fluid2 *x, void *b, long m, long a, char *s)
{
	if (m == 1) {
		switch(a)
		{
			case 0:
				sprintf(s, "(Matrix) Colour Field (2D, 4 plane, char)");
				break;
		...
		switch (a) {
			case 0:
				sprintf(s,"(Matrix) Updated Colour Field (same as inlet #1)");
				break; 			
			case 1:
				sprintf(s,"(Matrix) Updated Velocity Field (same as inlet #2)");
				break; 			
			case 2:
				sprintf(s,"dumpout");
				break;			
